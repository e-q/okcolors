...
class sRGB:
    ...
    def quantize(self) -> sRGB:
        return sRGB(
            round(self.r * 255) / 255,
            round(self.g * 255) / 255,
            round(self.b * 255) / 255,
        )
...
class ColorPalette:
    ...
    def quantize(self) -> ColorPalette:
        return ColorPalette(
            colors={name: c.to_srgb().quantize() for name, c in self.colors.items()},
            name=self.name,
        )
