...
# APCA uses a simple 2.4 power curve (monitor gamma), NOT the piecewise sRGB
# transfer function.
_MAIN_TRC = 2.4
...
def _apca_rgb_to_y(r: float, g: float, b: float) -> float:
    """Convert sRGB [0,1] to APCA luminance Y using simple 2.4 power curve."""
    return _Y_R * r**_MAIN_TRC + _Y_G * g**_MAIN_TRC + _Y_B * b**_MAIN_TRC


def _srgb_to_y(r: float, g: float, b: float) -> float:
    """Convert sRGB [0,1] to relative luminance Y (standard sRGB linearization)."""
    return (
...
def apca_contrast(fg_color: Color, bg_color: Color) -> float:
    ...
    """
    fg_srgb = fg_color.to_srgb().quantize()
    bg_srgb = bg_color.to_srgb().quantize()

    y_fg = _apca_rgb_to_y(fg_srgb.r, fg_srgb.g, fg_srgb.b)
    y_bg = _apca_rgb_to_y(bg_srgb.r, bg_srgb.g, bg_srgb.b)
